...
App.controller("mainCtrl", [
    ...
    function($scope, $http, $interval) {
		...
		$scope.toggleShow = function(what) {
			$scope.show[what] = !$scope.show[what];
		}
		$scope.showAll = function() {
			for (var foo in $scope.show) {
				$scope.show[foo] = true
			}
		}
		$scope.hideAll = function() {
			for (var foo in $scope.show) {
				$scope.show[foo] = false
			}
		}
		// $scope.showAll()

		function tick(timestamp) {
			if ($scope.runMode == "tick") {
			...
			// Calculate weight
			if ($scope.weight.blocked || !$scope.show.escapement) {
		...
		$scope.runMode = false;
		var lastMinuteTS = null;
		function spinMinuteHand(timestamp) {
			if ($scope.runMode == "minute") {
				window.requestAnimationFrame(spinMinuteHand);
			} else {
				lastMinuteTS = null;
				return;
			}
			if (lastMinuteTS === null) {
				lastMinuteTS = timestamp;
				return;
			}

			var diff = timestamp - lastMinuteTS;
			if (!$scope.minutespin) return

			deltaAngle("cog2", $scope.minutespin * diff/166.66/60)
		...
		var lastWeightTS = null
		function weightdrop(timestamp) {
			if ($scope.runMode == "weight") {
				window.requestAnimationFrame(weightdrop);
			} else {
				lastWeightTS = null;
				$scope.weight.velocity = 0;
				return;
			}
			if (lastWeightTS === null) {
				lastWeightTS = timestamp;
				return;
			}
			var timeDiffMilliseconds = timestamp - lastWeightTS;
			var timeDiffSeconds = timeDiffMilliseconds/1000;

			// Calculate weight
			if (false) { //$scope.weight.blocked) {
				// weight drop doesn't change.
				// vel goes to zero
				$scope.weight.velocity = 0;
			} else {
				// calc force on weight
				var forceG = $scope.weight.mass * 9.8;
				var forceFriction = ($scope.weight.frictionConst/100) + ($scope.weight.velocity * $scope.weight.velocity * $scope.weight.friction/10);

				var overallForce = forceG - forceFriction; // acts down
				var accel = overallForce / $scope.weight.mass;
				accel = Math.round(accel * 1000000)/1000000
				var velDelta = accel * timeDiffSeconds;
				var posDelta = ($scope.weight.velocity * timeDiffSeconds) + (0.5 * accel * timeDiffSeconds * timeDiffSeconds); //ut +0.5at^s
				$scope.weight.velocity += velDelta;
				// posDelta is in metres
				// ben's screen is roughly 0.18m per 800 logical pixels == 800/0.18 pixels per m == 4444ppm
				console.log(accel, $scope.weight.velocity, timeDiffSeconds, posDelta)
				deltaAngle("weightDrop", posDelta * 100);
				// deltaAngle("weightDrop", posDelta );
				// if ($scope.angles.weightDrop > 4000) {
				// 	$scope.weight.blocked = true;
				// }
		...
		$scope.changeRunMode = function(mode) {
			$scope.runMode = mode;
			if (mode == "minute") {
				window.requestAnimationFrame(spinMinuteHand);
			} else if (mode == "tick") {
